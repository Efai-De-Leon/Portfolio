...
void ProcessRequest(int DataSocketFD, fd_set ReadFDs, RUNLIST *runlist) {
...
#ifdef DEBUG
					printf("LOGIN: Append is successful\n");
#endif
				}
				else
				{
	...
	else
	{
		...
		if (strcmp(RecvBuf, "Client has requested to shutdown.") == 0) {
			...
			LoggingOut(DataSocketFD, runlist);	
		...
		if (SendToMessage(RecvBuf) == TRUE) {
			int check = WriteIntendedFriend(DataSocketFD, RecvBuf, runlist);
			if (check == 0) {
	...
	l = strlen(SendBuf);
	printf("LOOP: Sending response: %s.\n", SendBuf);
		...
		if (res < 0)
		{
			...
			for (i = 0; i<FD_SETSIZE; i++)
			{
				if (FD_ISSET(i, &ReadFDs))
				{
					...
					{
#ifdef DEBUG
						printf("LOOP: %s: New Client Attempting to connect...\n", Program);
#endif
...
#endif
						ProcessRequest(i, ActiveFDs, list);
						if (FlagDisconnect == 1) {
